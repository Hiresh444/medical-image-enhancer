"""Deterministic (non-LLM) agents for the medical imaging QA pipeline.

Five agents that form the core pipeline:
  1. QualityDetectionAgent — detect quality issues
  2. RecommendationAgent  — map issues to recommendations
  3. EnhancementAgent     — apply deterministic enhancement
  4. ValidationAgent      — compare original vs enhanced
  5. ReportAgent          — build markdown report
"""
...
from pipeline.metrics import compute_metrics, detect_issues, compute_validation
from pipeline.enhancement import apply_enhancements
from pipeline.dicom_io import build_markdown_report
...
class EnhancementAgent:
    def run(
        self, image: np.ndarray, recommendations: RecommendationResult
    ) -> EnhancementResult:
        enhanced, applied_ops = apply_enhancements(
            image, list(recommendations.mapping.keys())
        )
        metrics = compute_metrics(enhanced)
        return EnhancementResult(
            image=enhanced, applied_ops=applied_ops, metrics=metrics
        )
...
class ValidationAgent:
    def run(
        ...
        if status == "FAIL" and validation["quality_improvement"] > 0:
            status = "WARN"
            notes.append(
                "Some improvement observed, but thresholds not fully met."
            )

        if validation.get("niqe_improved"):
            notes.append(
                "Naturalness preserved (NIQE-approx stable or improved)."
            )
        else:
            notes.append(
                "Warning: Naturalness may be degraded (possible over-processing)."
            )

        if validation.get("noise_change", 0) > 0.5:
            notes.append(
                f"Note: Noise increased by {validation['noise_change'] * 100:.1f}% "
                f"(sharpening side-effect)."
            )
